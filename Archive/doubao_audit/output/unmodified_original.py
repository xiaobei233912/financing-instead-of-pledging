...
def simulate(data: pd.DataFrame, mode: str, p: Params = Params(),
    ...
    def mark(i, days):
        # 1) 单利计息（按自然日）
        if a.P > 0: a.I += a.P * p.margin_rate * days/365.0
    ...
    def sweep_out():
        ...
        x=min(removable,a.own_c); a.own_c-=x; a.o_c+=x; removable-=x
        x=min(removable,a.own_q); a.own_q-=x; a.o_q+=x
    ...
    def can_borrow_stress(amount):
        ...
        margin_gap = max(0.0, p.init_margin*D2 - own_after_haircut)
        # 初始保证金缺口也优先用现金补
        mc = min(max(0,a.o_c-need_c), margin_gap); mq = (margin_gap-mc)/p.haircut_q
        ...
        if Q > tgt + 1e-9:  # 股票超配 -> 卖股票换现金（必须恢复安全垫）
            ...
            if delta > 1e-9 and mode in ("C0","C1","C2"):
                ...
                if x>0:
                    ...
                    if a.own_haircut(p) >= p.init_margin*(a.P+x)-1e-9:
                        a.P+=x; a.fin_c+=x
                    else:
                ...
                if x>0:
                    a.own_q-=x; a.P=max(0,a.P-x); delta-=x
